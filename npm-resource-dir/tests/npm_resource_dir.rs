...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let css = reqwest::blocking::get(format!(
        "http://127.0.0.1:{port}/static/@fortawesome/fontawesome-free/css/all.css"
    ))?
    .text()?;
    let js = reqwest::blocking::get(format!(
        "http://127.0.0.1:{port}/static/@fortawesome/fontawesome-free/js/all.js"
    ))?
    .text()?;

    assert!(text.contains(r#"<i class="fas fa-thumbs-up"></i>"#));
    assert!(css.contains("Font Awesome Free"));
